...
pub fn render_result_into(
    ...
    match &result.typ {
        CalcResultType::Quantity(num, unit) => {
            ...
            return if let Some((final_unit, coeff)) = final_unit_and_coeff {
                if let Some(rust_is_a_joke) = num.checked_mul(&coeff) {
                    get_result(&rust_is_a_joke, &final_unit, f, decimal_count, use_grouping)
                } else {
                    get_result(num, unit, f, decimal_count, use_grouping)
                }
            } else {
                get_result(num, unit, f, decimal_count, use_grouping)
            };
        ...
        CalcResultType::Number(num) => num_to_string(f, num, format, decimal_count, use_grouping),
    ...
    for ch in cell_str.as_bytes() {
        ...
        if was_space {
            if only_digits_or_space_so_far && ch.is_ascii_digit() {
                // space was just a thousand separator
                int_part_len += 1;
                if unit_part_len > 0 {
